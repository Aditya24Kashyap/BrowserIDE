...
//     const { fileContent, cursorLine, cursorColumn, suggestionType, fileName, model } =
//       body;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = buildPrompt(context, suggestionType);
    const suggestion = await generateSuggestion(prompt, model || "qwen2.5-coder:1.5b");
...
async function generateSuggestion(prompt: string, model: string): Promise<string> {
  try {
    if (process.env.GEMINI_API_KEY) {
      ...
      const data = await response.json();
      let suggestion = data.candidates?.[0]?.content?.parts?.[0]?.text || "// AI suggestion unavailable";
      if (suggestion.includes("```")) {
        const codeMatch = suggestion.match(/```[\w]*\n?([\s\S]*?)```/);
        suggestion = codeMatch ? codeMatch[1].trim() : suggestion;
      }
